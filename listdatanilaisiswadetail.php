<?php
$ds = mysql_fetch_array(mysql_query("SELECT nis, nama_siswa FROM `data_siswa` WHERE nis = '".$_GET['nis']."'"));
/*
...
*/
$dk = mysql_fetch_array(mysql_query("select * from data_kelas where kode_kelas = '".$_GET['kelas']."'"));
?>
...
LEFT JOIN data_siswa s ON b.`kode_kelas` = s.`kode_kelas` AND b.`kode_kelas` = '".$_GET['kelas']."'
	  	WHERE nis = '".$_GET['nis']."'");
	  ...
	  //nilai angka pengetahuan ============================================================================================
						  		$dnp = mysql_fetch_array(mysql_query("SELECT SUM(uh1+uh2+uh3+uh4+uh5) AS total_uh, uts, uas FROM nilai_pengetahuan 
						  			WHERE nis = '".$_GET['nis']."' AND tahun_ajaran = '".$dbl['thn_ajaran']."' AND nip = '".$dbl['nip']."' AND kode_pelajaran = '".$dbl['kode_pelajaran']."' AND kode_kelas = '".$dbl['kode_kelas']."' AND semester = '".$_GET['semester']."'"));
...
//nilai sikap ===================================================================================================================
      ...
      <tr>
		...
		/*$nilaitotal = $nrtpengetahuan['nilai1'] + $nrtpengetahuan['nilai2'] + $nrtpengetahuan['nilai3'] + $nrtpengetahuan['nilai4'] + $nrtpengetahuan['nilai5'] + $nrtpengetahuan['nilaiutas'] + $nrtpengetahuan['nilaiuas'] + $nrtketerampilan['nilai1'] + $nrtketerampilan['nilai2'] + $nrtketerampilan['nilai3'] + $nrtketerampilan['nilai4'] + $nrtketerampilan['nilai5'] + $nrtketerampilan['nilaifolio'] + $nrtketerampilan['nilaipoyek'] + $nrtsikap['nilai1'] + $nrtsikap['nilai2'] + $nrtsikap['nilai3'] + $nrtsikap['nilai4'];
		$nilairatarata = $nilaitotal / 18;*/
		//echo $a."|";
		print number_format($nilairatarata5,2); //nilairatarata ?>        </td>
        </tr>
      <tr>
...
if($nilairatarata6 > 0) {		
$qcn = mysql_query("select * from data_rapor where nis = '".$_GET['nis']."' and kode_kelas = '".$dk['kode_kelas']."' and thn_ajaran = '".$_GET['tahun']."'");
...
ORDER BY nilai DESC ) 
FROM data_rapor WHERE kode_kelas = '".$dk['kode_kelas']."' and thn_ajaran = '".$_GET['tahun']."' )
) AS rangking
FROM data_rapor WHERE nis = '".$_GET['nis']."'");

				$draport = mysql_fetch_array($qrapor);
				$totalsiswakelasnya = mysql_num_rows(mysql_query("select id_rapor from data_rapor 
					where kode_kelas = '".$dk['kode_kelas']."' and thn_ajaran = '".$_GET['tahun']."'"));
		?>
         <?php
		$totalsiswa = mysql_num_rows(mysql_query("SELECT * FROM `data_siswa` WHERE kode_kelas = '".$dk['kode_kelas']."' AND tahun = '".$_GET['tahun']."'"));
		?>
...
<button type="button" class="btn" onclick="location.href='?mod=listdatanilaisiswaview&kelas=<?php echo $_GET['kelas']; ?>&tahun=<?php echo $_GET['tahun']; ?>&nip=<?php echo $_GET['nip']; ?>'">Kembali</button>