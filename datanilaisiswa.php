<?php
$ds = mysql_fetch_array(mysql_query("SELECT nis, nama_siswa, kode_kelas, tahun FROM `data_siswa` WHERE nis = '".$_SESSION["userid"]."'"));
/*
$dk = mysql_fetch_array(mysql_query("SELECT k.`nama_kelas`, b.thn_ajaran, k.kode_kelas FROM `data_belajar` b LEFT JOIN `data_kelas` k ON k.`kode_kelas` = b.`kode_kelas` WHERE nis = '".$_SESSION['userid']."' GROUP BY b.`kode_kelas`"));
*/
...
<script>
function raport(nis) {
    window.open("nilaisiswa.php?semester=<?php echo $_GET['semester']; ?>&id="+nis, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=800, height=600");
}
</script>
<table width="100%" border="0">
  <tr>
    <td align="center"><h3>NILAI RAPOR</h3></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
...
LEFT JOIN data_siswa s ON b.`kode_kelas` = s.`kode_kelas` AND b.`kode_kelas` = '".$ds['kode_kelas']."'
	  ...
	  //nilai angka pengetahuan ============================================================================================
						  		$dnp = mysql_fetch_array(mysql_query("SELECT SUM(uh1+uh2+uh3+uh4+uh5) AS total_uh, uts, uas FROM nilai_pengetahuan 
						  			WHERE nis = '".$_SESSION['userid']."' AND tahun_ajaran = '".$dbl['thn_ajaran']."' AND nip = '".$dbl['nip']."' AND kode_pelajaran = '".$dbl['kode_pelajaran']."' AND kode_kelas = '".$dbl['kode_kelas']."' AND semester = '".$_GET['semester']."'"));
...
$qcn = mysql_query("select * from data_rapor where nis = '".$_SESSION['userid']."' and kode_kelas = '".$dk['kode_kelas']."' 
...
$dcn = mysql_fetch_array($qcn);
		...
		if(isset($dcn['id_rapor']) && $dcn['id_rapor'] != "") { 
			if($dcn['nilai'] != $nilairatarata6) {
				...
				mysql_query("INSERT INTO `data_rapor` (`kode_kelas`,`thn_ajaran`,`nis`,`nilai`) VALUES ('".$dk['kode_kelas']."','".$thnajaran."','".$_SESSION['userid']."','$nilairatarata6')");
		}
...
FROM data_rapor WHERE nis = '".$_SESSION['userid']."'");
  ...
  <tr>
    <td><button type="button" class="btn" onclick="raport(<?php echo $_SESSION['userid']; ?>)">Cetak Nilai</button></td>
  </tr>
  <tr>
    <td></td>
  </tr>
</table>
<button type="button" class="btn" onclick="location.href='?mod=datanilaisiswasemester'">Kembali</button>